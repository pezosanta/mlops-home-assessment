# ML Repository Guideline
Originally, this repository was created for a job interview home assessment. 
...
However, I decided to make it public as a demonstration on ML repository best practices (or at least what I consider as best practices). Please note that this project does not aim to solve any real-world problems, the entire ML Model develpoment part is just a toy example, do not expect proper data/training pipelines, experiment tracking, deployment etc. 
...
│       ├── pre-commit.yaml
│       └── training.yaml
├── .venvs                          # Virtualenvs (gitignored)
│   ├── <your_venv_1>
│   ├── <your_venv_2>
│   ├── ...
├── data                            # Folder containing the different datasets for local training/testing
│   └── cifar_dataset
│       └── metadata.json
...
│   ├── install-requirements-build.sh
│   ├── install-requirements-cq.sh
│   └── install-requirements-dev.sh
├── src                             # Source folder for Python package(s)
...
│   │   ├── modeling                # Folder containing scripts for model training
│   │   │   ├── __init__.py
│   │   │   ├── model.py
│   │   │   ├── training.py
│   │   │   └── validation.py
...
│   │   ├── __init__.py
│   │   └── main.py
├── .gitattributes                  # Git configs (Git LFS, Git EOL fix etc.)             
├── .gitignore                      # Gitignore file
├── .pre-commit-config.yaml         # Pre-commit hooks related settings
├── poetry.lock                     # Poetry Lock file containing the package dependencies
├── pyproject.toml                  # Python packaging (Poetry) / pre-commit hooks related settings
├── README.md                       # This file
└── tox.ini                         # Pre-commit hooks (flake8) related
```

## 2. Prerequisites
For development, `Windows` and `Ubuntu 22.04 (WSL2)` platforms were being used.

### 2.1 Visual Studio Code (VSCode)
Some of the features (Dev Containers, Docker extension etc.) require to use [VSCode](https://code.visualstudio.com/) as your text editor. 

### 2.2. Python
For this project, `Python3.10` is used. Please use this version in your `Dev Containers` and `virtualenvs`.

### 2.3 Git
`git>=2.10` 

### 2.4. Docker
If you wish to build and run the Docker images on your local machine, then you will need `Docker` to be installed. 
- If you use `Ubuntu` (e.g.: `WSL2`) for development, see more information about Docker installation in the [official Docker installation guide](https://docs.docker.com/engine/install/ubuntu/).
- If you use `Windows` for development, install [Docker Desktop](https://www.docker.com/products/docker-desktop/) with `WSL2` (Windows Subsystem for Linux v2) backend. 

In both cases, you will end up developing in `Ubuntu` thanks to `Dev Containers`, so I recommend you to get familiar with `Bash` fundamentals.
